"""R2-S Axis 1 look-alike-intent stress runner — System C0 baseline.
...
def assign_failure_bucket(
    ...
    """Map a scored case to one of the four Axis 1 buckets.

    Mutually exclusive, exhaustive over all scored cases:

    - `wrong_adjacent_intent`: pred is a known intent ≠ gold, ≠ "unknown".
    - `unknown_intent`: pred == "unknown".
    - `guard_protected`: intent correct AND every downstream metric is
      `True` / `1.0` — the C0 guards (customer-number, listing-phrase
      precedence, family routing) held under attractor pressure.
    - `downstream_mismatch`: intent correct but at least one
      downstream metric is not perfect.

    The bucket name is recorded on `StressCaseResult.bucket` and is
    the primary axis-1-specific classification used by the report.
    """
    if predicted_intent == "unknown" and not intent_correct:
        return "unknown_intent"
    if not intent_correct:
        return "wrong_adjacent_intent"
    # intent_correct == True from here on
    downstream_perfect = (
        answerability_correct
        and behavior_class_correct
    ...
    return "guard_protected" if downstream_perfect else "downstream_mismatch"
...
class StressCaseResult:
    ...
    score_present: bool
    bucket: str  # wrong_adjacent_intent | unknown_intent | guard_protected | downstream_mismatch
    notes: str = ""
...
def _check_head(required: str) -> Optional[str]:
    ...
    return (
        ...
        f"Axis 1 C0 metrics are only comparable at the frozen baseline."
    )


def _build_result(
    case: Run2LookalikeCase,
    ...
    if score is None:
        ...
        bucket = assign_failure_bucket(
            ...
            answerability_correct=score.answerability_correct,
    ...
    return StressCaseResult(
        ...
        attractor_intent=case.attractor_intent,
...
def run_system_c0(
    ...
    """Run System C0 across the full 24-case Axis 1 stress split."""
    cases = load_lookalike_cases(cases_path)
    val = validate_all_lookalike_cases(cases)
    if val.n_errors:
        raise ValueError(
            f"axis1 stress CSV validation failed with {val.n_errors} "
...
def run_system_b_stub(*_args, **_kwargs) -> None:
    ...
    Not implemented at the Axis 1 baseline. The existing
    `run2_model_baseline_runner.py` knows how to drive Run 2 cases
    through OpenAI; the wiring here would mirror Axis 3's
    `run_system_b_stub` docstring."""
    raise NotImplementedError(
        "Axis 1 System B runner is deferred. See run2_model_baseline_runner."
    )


def run_system_a_stub(*_args, **_kwargs) -> None:
    """System A (prior + model fallback) stress run. Not implemented."""
    raise NotImplementedError(
        "Axis 1 System A runner is deferred. See run2_model_baseline_runner."
    )
...
def build_scatter_rows(artifacts: RunArtifacts) -> list[dict]:
    ...
    Per `shared/scatter_schema.md`:
      ...
      - system = "c0"
      - band = the case's `confusion_pair`
    ...
    skip.
    """
    scored_pairs: list[tuple[Run2LookalikeCase, Any]] = []
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="axis1_lookalike.runner",
        description=(
            "Run R2-S Axis 1 look-alike-intent stress on System C0 and "
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Import locally to avoid a circular dependency when report.py
