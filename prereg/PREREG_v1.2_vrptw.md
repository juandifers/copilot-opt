...
**Status:** LOCKED v1.2 — no further changes without versioned amendment.
**Author:** Juan
**Date drafted:** 2026-05-14
**Date locked (v1.0):** 2026-05-14T00:24:20+02:00
**Date locked (v1.1):** 2026-05-14
**Date locked (v1.2):** 2026-05-15
...
> This is the locked v1.2 pre-registration for the VRPTW track. v1.2 amends v1.1 in three places — (a) §12.1 OBJ-block adjudication for `OBJ × TIME_WINDOW` and `OBJ × TRAVEL_TIME` under §17, (b) §13/§14.1 specification that the headline AUROC evaluation unit is per-claim-family with degenerate-positive blocks reported informationally only, and (c) §3.3 cheap-action reconciliation with §13.1 — per the §17 amendment procedure. v1.1 amended v1.0 in one place (the definition of `reference_struct_unstable` for cells with no feasible reference on any seed). The v1.0 and v1.1 documents (`prereg/PREREG_v1.0_vrptw.md`, `prereg/PREREG_v1.1_vrptw.md`) remain as the prior locked specifications. v1.2 is the current authoritative spec for analyses run after the v1.2 lock; the thesis cites v1.0, v1.1, and v1.2 per §17. See §19 for the change log and the empirical and procedural reasons for the v1.2 amendment. The CVRP pre-registration (`PREREG_v0.5.md`) is unchanged and remains the locked specification for the CVRP track.
...
**Numerical sufficiency** is `band[cheap_action(perturbation_family), claim_family] == 'easy'`, where `cheap_action(perturbation_family)` is the family-specific cheap action defined in §13.1 (`reuse_direct` on non-OC families; `local_repair_insert` on ORDER_CHANGE). The fixed solution evaluated under the perturbation is numerically close to the reference for this claim family. This label is diagnostic only and is not the primary predictor target. Numerical closeness without operational validity is the failure mode the benchmark exists to expose. (v1.2 amendment: the v0.5/v1.0/v1.1 wording hard-coded `reuse_direct`, which conflicted with §13.1's family-specific cheap-action definition for ORDER_CHANGE. v1.2 defers cheap-action definition to §13.1 throughout §3.3. The long-table assembly already used the family-specific cheap action via the `is_cheap_action` column, so the amendment is documentation-only — no Stage A data changes.)

**Operational sufficiency** is claim-dependent. Throughout the four bullets below, `cheap_action` denotes `cheap_action(perturbation_family)` per §13.1.

- **OBJ:** `band[cheap_action, OBJ] == 'easy' AND feasibility_flag[cheap_action] == TRUE`. An objective answer that reports a cost number is only operationally honest if the underlying plan is executable. Numerical closeness with infeasibility is exactly the failure case the benchmark exposes; an answer that's "close" to the right cost on an unflyable plan is wrong, not approximately right. (Same rule as v0.5; the v1.2 wording switches from `reuse_direct` to `cheap_action(family)` to reconcile with §13.1.)

- **PLAN_VALIDITY:** `band[cheap_action, PLAN_VALIDITY] == 'easy'`, equivalently `feasibility_flag[cheap_action] == TRUE`. The label is true exactly when the cheap action's plan is itself executable. This is a substantive claim under v1.0: the 18-instance scale-check reports PLAN_VALIDITY-easy at 30% for `reuse_direct` and 50% for `local_repair_insert` on OC cells, so the label is not trivial. On OC, the cheap action is `local_repair_insert` per §13.1; on TT / TW / ST, it is `reuse_direct`.

- **STRUCT:** `band[cheap_action, STRUCT] == 'easy'`. Operational validity does not apply: a structural claim ("which customers move?") is answerable regardless of whether the underlying plan is feasible.

- **SCHEDULE:** `band[cheap_action, SCHEDULE] == 'easy'`. Same reasoning as STRUCT — a schedule-shift claim is answerable on the action's reported start times regardless of whether the plan is feasible.

**Structural sufficiency** is `band[cheap_action, STRUCT] == 'easy' AND band[cheap_action, SCHEDULE] == 'easy'`, computed cell-wise. It captures whether the fixed solution preserves both the route organisation and the schedule of the reference. Used as a secondary predictor target and in descriptive analyses; not a primary headline metric.

This claim-dependent definition is carried over from v0.5 §3.3 with the RANK → SCHEDULE substitution and the PLAN_VALIDITY semantic shift (positive control → substantive feasibility) folded in. The v1.2 amendment replaces `reuse_direct` with `cheap_action(perturbation_family)` throughout §3.3 to make the wording consistent with §13.1's family-specific cheap-action definition; under v0.5, `reuse_direct` was the only candidate cheap action and the two readings coincided.
...
**v1.2 amendment: `OBJ × TIME_WINDOW` and `OBJ × TRAVEL_TIME` adjudicated under §17.** Stage A's verification on `data/stage_a_vrptw_consolidated.parquet` (with the v1.1 stability rule applied) returned two §12.1 failures, both "too positive": `OBJ × TIME_WINDOW = 1.0000` (224 / 224 defined) and `OBJ × TRAVEL_TIME = 0.9727` (214 / 220 defined). The remaining fourteen blocks fell within `[0.10, 0.90]`. A diagnostic probe applied the Appendix A.1 / A.2 escalated grids to a stratified 20-instance matched sample (`data/probes/escalation_probe_report.md`, parquet sha256[:16] `689ec9984713be51`): on the matched sample, `OBJ × TW` moved from 1.0000 → 0.9625 and `OBJ × TT` from 0.9481 → 0.9342 — both remain above 0.90 after the one rung of escalation that §12.6 permits. The six currently-passing TT / TW non-OBJ blocks stayed inside `[0.10, 0.90]` under escalation; one additional TT cell became all-infeasible (3 → 4 of 80). Per the probe's verdict and the §12.6 cap of one substitution per block, the §12.6 §12.1-clause is exhausted for these two blocks with the threshold uncleared. Under v1.2 the substitution is **not applied** to `OBJ × TIME_WINDOW` or `OBJ × TRAVEL_TIME`. The OBJ saturation on TT / TW is characterized as a structural property of Solomon-100 under the locked OBJ-easy band (≤5% objective gap on the cheap action `reuse_direct`, which returns the unperturbed baseline routes): the perturbed-instance optimum stays within 5% of the unperturbed baseline distance on > 93% of cells at both grids, so `reuse_direct` is "easy" by the locked OBJ band regardless of which grid is in effect. This is corroborated by §12.5's feasibility-decoupling diagnostic reading 0.6217 on the full Stage A pool — the cheap action is OBJ-easy and operationally infeasible simultaneously on > 60% of cells, which is the failure mode the three-axis decomposition was designed to expose. The `[0.10, 0.90]` bracket is retained unchanged as a general non-degeneracy guard for the other fourteen blocks; for `OBJ × TW` and `OBJ × TT` specifically, the bracket is acknowledged to be miscalibrated for the OBJ family on soft Solomon-100 perturbations. The §13 / §14.1 amendment below carries the downstream consequence (per-claim-family AUROC with these two blocks flagged degenerate-positive and reported informationally only). No threshold is changed by v1.2 — §17 forbids threshold amendments in response to observed data; the [0.10, 0.90] bracket stays locked.
...
**v1.2 amendment: evaluation unit is per-claim-family.** v1.0/v1.1 §14.1 specified "AUROC on operational_sufficiency" without fixing whether AUROC is computed per-claim-family (pooling all four perturbation families within each claim) or per-(claim_family × perturbation_family) block (one AUROC per of the 16 blocks). v1.2 amends §13 / §14.1 to specify that **the headline AUROC is computed per claim family, pooling the four perturbation families within each claim**. Four headline AUROCs are reported (OBJ, PLAN_VALIDITY, STRUCT, SCHEDULE), each with bootstrap 95% CIs over the 56 LOIO folds. The pooled class imbalance per claim family is disclosed in the methods section.

This amendment is the v1.2 §12.1 adjudication's downstream consequence: `OBJ × TIME_WINDOW` and `OBJ × TRAVEL_TIME` are degenerate-positive blocks (>90% positive class). Pooling the four perturbation families within OBJ gives 808 positive and 81 negative cells (n=896, ~9% negatives, excluding 7 §8.3 all-infeasible cells under v1.1), which is class-imbalanced but discriminable. Per-block descriptive results for the two degenerate-positive blocks are reported as informational only — flagged as degenerate (≤3 negative cells each in the matched pool) — and no within-block AUROC is computed for them. The other twelve `(claim_family × perturbation_family)` blocks continue to support within-block descriptive analysis as planned, alongside the four headline per-claim-family AUROCs.

The choice of "per-claim-family pooled" over "per-block" as the evaluation unit was *available to be made* before Stage A was collected — both readings were consistent with v1.0/v1.1's §14.1 wording. Under §17, this is a substantive amendment of an under-specified clause rather than a post-hoc threshold change. The clarification is gated on (and motivated by) the §12.1 OBJ adjudication above, and is recorded under §17's amendment-outside-deterministic-menu authority. No thresholds are altered.
...
- **v1.2 (2026-05-15, LOCKED):** Amendment. Three changes: (a) §12.1 adjudication of two failing OBJ blocks (`OBJ × TIME_WINDOW`, `OBJ × TRAVEL_TIME`) under §17 — the §12.6 §12.1-clause's one permitted substitution is exhausted with the threshold uncleared, and the substitution is not applied; (b) §13 / §14.1 specification that the headline AUROC evaluation unit is per claim family (pooling the four perturbation families within each claim) with degenerate-positive blocks reported informationally only; (c) §3.3 wording reconciled with §13.1 — `cheap_action` is family-specific throughout §3.3 instead of hard-coded `reuse_direct`.

  *Data state at the time of the amendment.* Stage A has been collected under v1.0 (`data/stage_a_vrptw.parquet`) and consolidated under v1.1 (`data/stage_a_vrptw_consolidated.parquet`, `data/stage_a_vrptw_consolidated_claim_rows.parquet`). The §12.1, §12.2, §12.3, §12.5 verification checks have all been run against the consolidated artifact (`data/stage_a_vrptw_verification_report.md`). §12.2, §12.3, §12.5 pass. §12.1 fails on two OBJ blocks (`TW = 1.0000`, `TT = 0.9727`). A diagnostic probe of the Appendix A.1 / A.2 escalation has been run (`data/probes/escalation_probe_report.md`, parquet sha256[:16] `689ec9984713be51`). **No predictor has been trained.** No headline AUROC has been computed. v1.2 is locked before any model is fit, so the per-claim-family vs per-block evaluation-unit specification is a pre-registered choice, not a post-hoc selection in response to model performance.

  *Sections being amended.* §3.3 (sufficiency labels), §12.1 (label-distribution check), §13 / §14.1 (predictor scope and metrics). §17 (amendment procedure) and Appendix A (revision menu) are unchanged. No thresholds are altered.

  *Old text (v1.1 §3.3, numerical sufficiency).*
  > Numerical sufficiency is `band[reuse_direct, claim_family] == 'easy'`.

  *New text (v1.2 §3.3, numerical sufficiency).*
  > Numerical sufficiency is `band[cheap_action(perturbation_family), claim_family] == 'easy'`, where `cheap_action(perturbation_family)` is the family-specific cheap action defined in §13.1 (`reuse_direct` on non-OC families; `local_repair_insert` on ORDER_CHANGE).

  The four operational-sufficiency bullets in §3.3 (OBJ, PLAN_VALIDITY, STRUCT, SCHEDULE) receive the same `reuse_direct` → `cheap_action(perturbation_family)` substitution, since they all referenced the same v0.5 carry-over wording.

  *Old text (v1.1 §12.1).* Specifies the `[0.10, 0.90]` bracket without an explicit clause for the case where §12.6's one-rung-per-block escalation cannot clear a too-positive block.

  *New text (v1.2 §12.1).* Adds an "OBJ-block adjudication" paragraph documenting the §17 decision *not* to apply the §12.6 substitution to `OBJ × TIME_WINDOW` or `OBJ × TRAVEL_TIME`, with the matched-sample probe evidence and the structural-saturation characterization. **The `[0.10, 0.90]` bracket itself is unchanged.**

  *Old text (v1.1 §14.1, headline metric).* "AUROC on operational_sufficiency. Headline metric." Silent on whether AUROC is per-claim-family or per-(claim_family × perturbation_family) block.

  *New text (v1.2 §14.1).* Adds an "evaluation unit is per-claim-family" paragraph specifying four headline AUROCs (one per claim family, pooling the four perturbation families within each), with bootstrap 95% CIs over the 56 LOIO folds, and a note that degenerate-positive blocks (`OBJ × TW`, `OBJ × TT`) are reported as informational only with no within-block AUROC.

  *Empirical reason for the amendment.* §12.1's verification on the consolidated Stage A artifact returned `OBJ × TW = 1.000` and `OBJ × TT = 0.973`, both above the 0.90 ceiling. The diagnostic probe at the Appendix A escalation rung moved these by −0.0375 and −0.0138 on a matched 20-instance sample (`escalation_probe_report.md` §3, §4). Both blocks remain saturated. The §12.6 menu permits one substitution per block; consuming it would leave the failure in place and force Stage A re-collection per §12.6's "exactly one revision pass" clause. The §17 adjudication is the option that preserves the bracket as a non-degeneracy guard for the other fourteen blocks while routing the two-block failure into the predictor-scope amendment, where degenerate-positive blocks become an explicit reporting convention rather than a verification block.

  The §3.3 reconciliation is a separate, documentation-only fix: the v0.5 wording hard-coded `reuse_direct` because that was the only cheap action under consideration; the v1.0 expanded portfolio added `local_repair_insert` as the OC cheap action and §13.1 was updated accordingly, but §3.3's wording lagged. The §12.1 verification report flagged the ambiguity (Reading A under §3.3-literal gave `OC × PV = 0.000`; Reading B under §13.1 gave `OC × PV = 0.6696`). The long-table assembly already uses the family-specific cheap action via the `is_cheap_action` column (confirmed at v1.2 pre-launch: OC × PV under `is_cheap_action=True` is 224/224 defined, 150 easy, rate 0.6696). v1.2 makes §3.3's wording match the code.

  *Why the change does not undermine the pre-registration's purpose.* The `[0.10, 0.90]` bracket is unchanged. The 0.20 §12.5 threshold is unchanged. The 0.25 §12.3 threshold is unchanged. The five-action portfolio, four claim families, sixteen perturbation cells, LOIO protocol, and feature list are all unchanged. The amendment narrows the headline-metric reporting convention to a per-claim-family AUROC (which §14.1's "AUROC on operational_sufficiency" was already consistent with — neither reading was excluded by v1.0/v1.1 wording) and routes two specific block-level failures through an explicit informational-only reporting flag instead of a verification halt. No predictor has been trained, so the per-claim-family choice is a pre-registered specification, not a post-hoc model-selection lever. The §3.3 wording fix has no Stage A data consequence — the long-table assembly's `is_cheap_action` column already implements §13.1.

  *Why this is a §17 amendment, not a §12.6 application.* §12.6's deterministic menu (Appendix A) was tested by the diagnostic probe and shown to be insufficient for `OBJ × TW` and `OBJ × TT` on Solomon-100. §17 governs amendments outside the deterministic menu and explicitly permits substantive amendments with documented data state and stated reasons. The §17 amendment does not change thresholds (which §17 forbids); it changes which downstream consequence the bracket-violation routes into. The choice between (i) applying the §12.6 substitution anyway and accepting the residual failure, and (ii) the §17 amendment route, is documented in `data/probes/escalation_probe_report.md` §5 as the post-probe decision; v1.2 takes route (ii).

  *Headline-hypothesis impact statement (per §17).* H1a, H1b, H2, H3, H4 are unchanged. The amendment alters the *reporting unit* for AUROC (four per-claim-family AUROCs instead of one pooled or sixteen per-block) but not the metrics themselves or the pass/fail criteria attached to them. The thesis discussion will note the §12.1 adjudication and the per-claim-family reporting choice when reporting H1a/H1b AUROC results, per §17's "amendments that affect the headline hypotheses require an explicit explanation in the thesis discussion" clause — even though §17 strictly speaking refers to changes that alter what is being tested, not how it is reported.

  *Code change accompanying the amendment.* None. v1.2 pre-launch verification (step 4) confirmed that the long-table assembly's `is_cheap_action` column already uses the family-specific cheap action; OC × PV under `is_cheap_action=True` reads 0.6696 in `data/stage_a_vrptw_consolidated_claim_rows.parquet`, matching §13.1. The §3.3 amendment is documentation-only.

  *Tagging.* This entry is locked together with the file; the v1.2 lock commit gets tag `prereg-v1.2-vrptw` per §17. The v1.0 and v1.1 tags are unchanged and continue to mark their respective locked specifications.
