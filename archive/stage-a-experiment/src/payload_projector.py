...
def run_action(dataset: str, instance_id: str, perturbation_id: str,
    ...
    baseline_eval = evaluate_vrptw_solution(instance, baseline.routes)
    ...
    projected = ProjectedAction(
        ...
        affected_customers=perturbed.affected_customers,
...
def _project_baseline_solution(family: str, projected: ProjectedAction) -> dict:
    """Project the baseline (pre-perturbation) evaluation into family-specific shape."""
    bev = projected.baseline_evaluation
    if bev is None:
        return {}
    if family == "OBJ":
        return {
            "objective": round(float(bev.objective) / SCALING_FACTOR, 2),
        }
    if family == "PLAN_VALIDITY":
        return {
            "feasible": bool(bev.feasible),
            "feasibility_breakdown": {
                "capacity_ok": bool(bev.feasible_capacity_only),
                "time_windows_ok": bool(bev.feasible_tw_only),
                "coverage_ok": bool(bev.is_complete),
            },
        }
    if family == "STRUCT":
        routes = []
        for rs in bev.route_summaries:
            if rs.n_customers == 0:
                continue
            ridx = int(rs.route_idx)
            cids = [int(c) for c, vs in bev.per_customer_schedule.items()
                    if int(vs.route_idx) == ridx]
            cids = sorted(cids, key=lambda c: int(bev.per_customer_schedule[c].start_service))
            if cids:
                routes.append({"route_idx": ridx, "customer_ids": cids})
        return {"n_routes": len(routes), "routes": routes}
    if family == "SCHEDULE":
        late_ids = sorted(
            int(c) for c, vs in bev.per_customer_schedule.items()
            if int(vs.time_warp) > 0
        )
        customer_schedule = []
        for cid, vs in sorted(bev.per_customer_schedule.items()):
            customer_schedule.append({
                "customer_id": int(cid),
                "route_idx": int(vs.route_idx),
                "arrival": round(float(vs.arrival) / SCALING_FACTOR, 1),
...
def _compute_diff(family: str, projected: ProjectedAction) -> dict:
    """Compute family-specific diff between baseline and action evaluations."""
    bev = projected.baseline_evaluation
    aev = projected.evaluation
    if bev is None:
        return {}

    if family == "OBJ":
        b_obj = round(float(bev.objective) / SCALING_FACTOR, 2)
        a_obj = round(float(aev.objective) / SCALING_FACTOR, 2)
        delta = round(a_obj - b_obj, 2)
        pct = round(100.0 * (a_obj - b_obj) / b_obj, 2) if b_obj != 0 else 0.0
        return {
            "objective": {
    ...
    if family == "PLAN_VALIDITY":
        b_feas = bool(bev.feasible)
        a_feas = bool(aev.feasible)
        return {
            "feasibility": {
                "became_infeasible": b_feas and not a_feas,
                "became_feasible": not b_feas and a_feas,
            },
        }

    if family == "STRUCT":
        b_route_idxs = {int(rs.route_idx) for rs in bev.route_summaries
                        if rs.n_customers > 0}
        a_route_idxs = {int(rs.route_idx) for rs in aev.route_summaries
                        if rs.n_customers > 0}
        b_assign = {int(c): int(vs.route_idx)
                    for c, vs in bev.per_customer_schedule.items()}
        a_assign = {int(c): int(vs.route_idx)
                    for c, vs in aev.per_customer_schedule.items()}
        modified = []
        for ridx in sorted(b_route_idxs & a_route_idxs):
            b_cids = {c for c, r in b_assign.items() if r == ridx}
            a_cids = {c for c, r in a_assign.items() if r == ridx}
            added = sorted(a_cids - b_cids)
            removed = sorted(b_cids - a_cids)
            if added or removed:
                modified.append({
                    "route_idx": ridx,
                    "customers_added": added,
                    "customers_removed": removed,
                })
        return {
            "routes": {
                "added": sorted(a_route_idxs - b_route_idxs),
                "removed": sorted(b_route_idxs - a_route_idxs),
                "modified": modified,
            },
        }

    if family == "SCHEDULE":
        b_late = {int(c) for c, vs in bev.per_customer_schedule.items()
                  if int(vs.time_warp) > 0}
        a_late = {int(c) for c, vs in aev.per_customer_schedule.items()
                  if int(vs.time_warp) > 0}
        return {
            "schedule": {
                "new_late_customer_ids": sorted(a_late - b_late),
                "no_longer_late_customer_ids": sorted(b_late - a_late),
            },
        }
...
def _cross_family_fields(projected: ProjectedAction) -> dict:
    """Emit fields required across all families: assignment and new_customer_ids."""
    ev = projected.evaluation
    assignment = {
        str(int(cid)): int(vs.route_idx)
        for cid, vs in ev.per_customer_schedule.items()
    }
    if projected.perturbation_family == "ORDER_CHANGE":
        new_customer_ids = [int(c) for c in projected.affected_customers]
    else:
        new_customer_ids = []
    return {
...
def build_payload(dataset: str, instance_id: str, perturbation_id: str,
    ...
    payload = project_payload(family, projected)
    payload.update(_cross_family_fields(projected))
    payload["baseline_solution"] = _project_baseline_solution(family, projected)
    payload["diff"] = _compute_diff(family, projected)
    return payload
